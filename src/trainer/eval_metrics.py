...
def _is_fsdp_model(m) -> bool:
    FSDP = _get_fsdp_class()
    return FSDP is not None and isinstance(m, FSDP)


def _unwrap_model(m):
    """
    Trainer may wrap the model in DDP. Keep FSDP wrappers intact so sharding stays active.
    """
    if _is_fsdp_model(m):
        return m
...
def _fsdp_full_param_context(model):
    """
    Return (module, context_manager) to work with full params when model is FSDP-wrapped.
    """
    if _is_fsdp_model(model):
        module = model.module
        return module, model.summon_full_params(module, writeback=False, recurse=False)
...
def _generate_assistant_turn(
    ...
    generate_model, full_param_ctx = _fsdp_full_param_context(model)
    with full_param_ctx:
        gen_ids = generate_model.generate(
            input_ids=input_ids,
            attention_mask=attention_mask,
            max_new_tokens=max_new_tokens,
            do_sample=(temperature > 0.0),
            temperature=(temperature if temperature > 0.0 else None),
            pad_token_id=getattr(generate_model.config, "pad_token_id", tokenizer.eos_token_id),
